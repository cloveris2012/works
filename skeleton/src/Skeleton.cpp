...
Skeleton::Skeleton(SkeletonData *skeletonData, SkeletonObjectCreator &creator) :
		_data(skeletonData),
		_skin(NULL),
		_color(1, 1, 1, 1),
		_time(0),
		...
		_x(0),
		_y(0) {
	_matrices.resize(_data->getBones().size() * 9);
	...
	for (size_t i = 0; i < _data->getBones().size(); ++i) {
		...
		if (data->getParent() == NULL) {
			bone = creator.createBone(*data, *this, NULL);
		} else {
			Bone *parent = _bones[data->getParent()->getIndex()];
			bone = creator.createBone(*data, *this, parent);
	...
	for (size_t i = 0; i < _data->getSlots().size(); ++i) {
		...
		Slot *slot = creator.createSlot(*data, *bone);
	...
	for (size_t i = 0; i < _data->getIkConstraints().size(); ++i) {
		...
		IkConstraint *constraint = creator.createIkConstraint(*data, *this);
	...
	for (size_t i = 0; i < _data->getTransformConstraints().size(); ++i) {
		...
		TransformConstraint *constraint = creator.createTransformConstraint(*data, *this);
	...
	for (size_t i = 0; i < _data->getPathConstraints().size(); ++i) {
		...
		PathConstraint *constraint = creator.createPathConstraint(*data, *this);
...
std::vector<float> &Skeleton::getMatrices(bool refresh) {
	if (refresh) {
		for (size_t i = 0, n = _bones.size(); i < n; ++i) {
			_matrices[i * 9 + 0] = _bones[i]->getA();
			_matrices[i * 9 + 1] = _bones[i]->getC();
			_matrices[i * 9 + 2] = 0;

			_matrices[i * 9 + 3] = _bones[i]->getB();
			_matrices[i * 9 + 4] = _bones[i]->getD();
			_matrices[i * 9 + 5] = 0;

			_matrices[i * 9 + 6] = _bones[i]->getWorldX();
			_matrices[i * 9 + 7] = _bones[i]->getWorldY();
			_matrices[i * 9 + 8] = 1;
		}
	}
	return _matrices;
