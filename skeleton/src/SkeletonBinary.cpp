...
SkeletonData *SkeletonBinary::readSkeletonData(const unsigned char *binary, const int length, SkeletonObjectCreator &creator) {
	bool nonessential;
    ...
    if ("3.8.75" == skeletonData->_version) {
        delete input;
        delete skeletonData;
        setError("Unsupported skeleton data, please export with a newer version of Skel.", "");
        return NULL;
    }
	...
	/* Default skin. */
	Skin* defaultSkin = readSkin(creator, input, true, skeletonData, nonessential);
	if (defaultSkin) {
	...
	for (size_t i = 0, n = (size_t)readVarint(input, true); i < n; ++i)
		skeletonData->_skins.add(readSkin(creator, input, false, skeletonData, nonessential));
...
SkeletonData *SkeletonBinary::readSkeletonDataFile(const String &path, SkeletonObjectCreator &creator) {
	int length;
	SkeletonData *skeletonData;
	const char *binary = SkelExtension::readFile(path.buffer(), &length);
	if (length == 0 || !binary) {
		setError("Unable to read skeleton file: ", path.buffer());
		return NULL;
	}
	skeletonData = readSkeletonData((unsigned char *) binary, length, creator);
	SkelExtension::free(binary, __FILE__, __LINE__);
	return skeletonData;
}
...
char *SkeletonBinary::readString(DataInput *input) {
	...
	string = SkelExtension::alloc<char>(length, __FILE__, __LINE__);
...
Skin *SkeletonBinary::readSkin(SkeletonObjectCreator &creator, DataInput *input, bool defaultSkin, SkeletonData *skeletonData, bool nonessential) {
	Skin *skin;
	int slotCount = 0;
	if (defaultSkin) {
	...
	for (int i = 0; i < slotCount; ++i) {
		...
		for (int ii = 0, nn = readVarint(input, true); ii < nn; ++ii) {
			...
			Attachment *attachment = readAttachment(creator, input, skin, slotIndex, name, skeletonData, nonessential);
...
Attachment *SkeletonBinary::readAttachment(SkeletonObjectCreator &creator, DataInput *input, Skin *skin, int slotIndex, const String &attachmentName,
	...
	case AttachmentType_Region: {
		...
		RegionAttachment *region = _attachmentLoader->newRegionAttachment(creator, *skin, String(name), String(path));
		region->_path = path;
	...
	case AttachmentType_Boundingbox: {
		...
		BoundingBoxAttachment *box = _attachmentLoader->newBoundingBoxAttachment(creator, *skin, String(name));
	...
	case AttachmentType_Mesh: {
		...
		mesh = _attachmentLoader->newMeshAttachment(creator, *skin, String(name), String(path));
		mesh->_path = path;
	...
	case AttachmentType_Linkedmesh: {
		...
		MeshAttachment *mesh = _attachmentLoader->newMeshAttachment(creator, *skin, String(name), String(path));
		mesh->_path = path;
		readColor(input, mesh->getColor());
	...
	case AttachmentType_Path: {
		PathAttachment *path = _attachmentLoader->newPathAttachment(creator, *skin, String(name));
	...
	case AttachmentType_Point: {
		PointAttachment *point = _attachmentLoader->newPointAttachment(creator, *skin, String(name));
	...
	case AttachmentType_Clipping: {
		...
		int vertexCount = readVarint(input, true);
		ClippingAttachment *clip = _attachmentLoader->newClippingAttachment(creator, *skin, name);
...
Animation *SkeletonBinary::readAnimation(const String &name, DataInput *input, SkeletonData *skeletonData) {
	...
	if (eventCount > 0) {
		...
		for (int i = 0; i < eventCount; ++i) {
			...
			if (freeString) SkelExtension::free(event_stringValue, __FILE__, __LINE__);
