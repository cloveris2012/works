...
SkeletonData *SkeletonJson::readSkeletonDataFile(const String &path, SkeletonObjectCreator& creator) {
	int length;
	SkeletonData *skeletonData;
	const char *json = SkelExtension::readFile(path, &length);
	if (length == 0 || !json) {
		setError(NULL, "Unable to read skeleton file: ", path);
		return NULL;
	}

	skeletonData = readSkeletonData(json, creator);

	SkelExtension::free(json, __FILE__, __LINE__);

	return skeletonData;
}

SkeletonData *SkeletonJson::readSkeletonData(const char *json, SkeletonObjectCreator& creator) {
	int i, ii;
	SkeletonData *skeletonData;
	...
	skeletonData = creator.createSkeletonData();
	...
	if (skeleton) {
		...
		/*if ("3.8.75" == skeletonData->_version) {
			delete skeletonData;
			setError(root, "Unsupported skeleton data, please export with a newer version of Skel.", "");
			return NULL;
		}*/
	...
	for (boneMap = bones->_child, i = 0; boneMap; boneMap = boneMap->_next, ++i) {
		...
		data = creator.createBoneData(bonesCount, Json::getString(boneMap, "name", 0), parent, boneMap);
	...
	if (slots) {
		...
		for (slotMap = slots->_child, i = 0; slotMap; slotMap = slotMap->_next, ++i) {
			...
			data = creator.createSlotData(i, Json::getString(slotMap, "name", 0), *boneData);
	...
	if (ik) {
		...
		for (constraintMap = ik->_child, i = 0; constraintMap; constraintMap = constraintMap->_next, ++i) {
			...
			IkConstraintData *data = creator.createIkConstraintData(Json::getString(constraintMap, "name", 0));
	...
	if (transform) {
		...
		for (constraintMap = transform->_child, i = 0; constraintMap; constraintMap = constraintMap->_next, ++i) {
			...
			TransformConstraintData *data = creator.createTransformConstraintData(Json::getString(constraintMap, "name", 0));
			data->setOrder(Json::getInt(constraintMap, "order", 0));
			data->setSkinRequired(Json::getBoolean(constraintMap, "skin", false));
	...
	if (path) {
		...
		for (constraintMap = path->_child, i = 0; constraintMap; constraintMap = constraintMap->_next, ++i) {
			...
			PathConstraintData *data = creator.createPathConstraintData(Json::getString(constraintMap, "name", 0));
			data->setOrder(Json::getInt(constraintMap, "order", 0));
			data->setSkinRequired(Json::getBoolean(constraintMap, "skin", false));
	...
	if (skins) {
		...
		for (skinMap = skins->_child, i = 0; skinMap; skinMap = skinMap->_next, ++i) {
			...
			Skin *skin = creator.createSkin(Json::getString(skinMap, "name", ""));
			...
			for (attachmentsMap = Json::getItem(skinMap, "attachments")->_child; attachmentsMap; attachmentsMap = attachmentsMap->_next) {
				...
				for (attachmentMap = attachmentsMap->_child; attachmentMap; attachmentMap = attachmentMap->_next) {
					...
					else if (strcmp(typeString, "skinnedmesh") == 0) type = AttachmentType_Mesh;
					...
					switch (type) {
						case AttachmentType_Region: {
							attachment = _attachmentLoader->newRegionAttachment(creator, *skin, attachmentName, attachmentPath);
							if (!attachment) {
								delete skeletonData;
						...
						case AttachmentType_Linkedmesh: {
							attachment = _attachmentLoader->newMeshAttachment(creator, *skin, attachmentName, attachmentPath);
							...
							if (!entry) {
								...
								LinkedMesh *linkedMesh = creator.createLinkedMesh(mesh,
						...
						case AttachmentType_Boundingbox: {
							attachment = _attachmentLoader->newBoundingBoxAttachment(creator, *skin, attachmentName);
						...
						case AttachmentType_Path: {
							attachment = _attachmentLoader->newPathAttachment(creator, *skin, attachmentName);
						...
						case AttachmentType_Point: {
							attachment = _attachmentLoader->newPointAttachment(creator, *skin, attachmentName);
						...
						case AttachmentType_Clipping: {
							attachment = _attachmentLoader->newClippingAttachment(creator, *skin, attachmentName);
	...
	if (events) {
		...
		for (eventMap = events->_child, i = 0; eventMap; eventMap = eventMap->_next, ++i) {
			EventData *eventData = creator.createEventData(String(eventMap->_name));
	...
	if (animations) {
		...
		for (animationMap = animations->_child; animationMap; animationMap = animationMap->_next) {
			Animation *animation = readAnimation(animationMap, skeletonData, creator);
			if (!animation) {
				delete skeletonData;
				delete root;
				return NULL;
...
SkeletonData *SkeletonJson::readSkeletonDataWithSkin(const char *json, std::map<std::string, std::string> skinList, SkeletonObjectCreator& creator) {
	int i, ii;
	SkeletonData *skeletonData;
	Json *root, *skeleton, *bones, *boneMap, *ik, *transform, *path, *slots, *skins, *animations, *events;
	...
	if (skins) {
		Json *skinMap;
		skeletonData->_skins.ensureCapacity(skins->_size + skinList.size());
		skeletonData->_skins.setSize(skins->_size + skinList.size(), 0);
		int skinsIndex = 0;
		for (skinMap = skins->_child, i = 0; skinMap; skinMap = skinMap->_next, ++i) {
			Json *attachmentsMap;
			Json *curves;
		...
		for (it = skinList.begin(); it != skinList.end(); it++) {
			extraSkinMap = new(__FILE__, __LINE__) Json(it->second.c_str());
			Json *attachmentsMap;
			Json *curves;

			Skin *skin = creator.createSkin(Json::getString(extraSkinMap, "name", ""));

			Json *item = Json::getItem(extraSkinMap, "bones");
			if (item) {
				for (item = item->_child; item; item = item->_next) {
					BoneData* data = skeletonData->findBone(item->_valueString);
					if (!data) {
						delete skeletonData;
						setError(extraSkinMap, String("Skin bone not found: "), item->_valueString);
						return NULL;
					}
					skin->getBones().add(data);
				}
			...
			item = Json::getItem(extraSkinMap, "ik");
			if (item) {
				for (item = item->_child; item; item = item->_next) {
					IkConstraintData* data = skeletonData->findIkConstraint(item->_valueString);
					if (!data) {
						delete skeletonData;
						setError(extraSkinMap, String("Skin IK constraint not found: "), item->_valueString);
						return NULL;
					}
					skin->getConstraints().add(data);
				}
			...
			item = Json::getItem(extraSkinMap, "transform");
			if (item) {
				for (item = item->_child; item; item = item->_next) {
					TransformConstraintData* data = skeletonData->findTransformConstraint(item->_valueString);
					if (!data) {
						delete skeletonData;
						setError(extraSkinMap, String("Skin transform constraint not found: "), item->_valueString);
						return NULL;
					}
					skin->getConstraints().add(data);
				}
			...
			item = Json::getItem(extraSkinMap, "path");
			if (item) {
				for (item = item->_child; item; item = item->_next) {
					PathConstraintData* data = skeletonData->findPathConstraint(item->_valueString);
					if (!data) {
						delete skeletonData;
						setError(extraSkinMap, String("Skin path constraint not found: "), item->_valueString);
						return NULL;
					}
					skin->getConstraints().add(data);
				}
			...
			skeletonData->_skins[skinsIndex++] = skin;
			if (strcmp(Json::getString(extraSkinMap, "name", ""), "default") == 0) {
				skeletonData->_defaultSkin = skin;
			}

			auto skinName = Json::getString(extraSkinMap, "name", "");

			for (attachmentsMap = Json::getItem(extraSkinMap, "attachments")->_child; attachmentsMap; attachmentsMap = attachmentsMap->_next) {
				SlotData* slot = skeletonData->findSlot(attachmentsMap->_name);
				Json *attachmentMap;
				...
				for (attachmentMap = attachmentsMap->_child; attachmentMap; attachmentMap = attachmentMap->_next) {
					...
					if (strcmp(typeString, "region") == 0) type = AttachmentType_Region;
						...
						delete skeletonData;
						setError(extraSkinMap, "Unknown attachment type: ", typeString);
						return NULL;
					}
					...
					case AttachmentType_Region: {
						...
						if (!attachment) {
							delete skeletonData;
							setError(extraSkinMap, "Error reading attachment: ", skinAttachmentName);
							return NULL;
						}
...
Animation *SkeletonJson::readAnimation(Json *root, SkeletonData *skeletonData, SkeletonObjectCreator &creator) {
	...
	for (slotMap = slots ? slots->_child : 0; slotMap; slotMap = slotMap->_next) {
		...
		for (timelineMap = slotMap->_child; timelineMap; timelineMap = timelineMap->_next) {
			if (strcmp(timelineMap->_name, "attachment") == 0) {
				AttachmentTimeline *timeline = creator.createAttachmentTimeline(timelineMap->_size, timelineMap);
				...
				for (valueMap = timelineMap->_child, frameIndex = 0; valueMap; valueMap = valueMap->_next, ++frameIndex) {
					...
					// todo
					String attachmentName = (name == NULL || name->_type == Json::JSON_NULL) ? "" : name->_valueString;
				...
				ColorTimeline *timeline = creator.createColorTimeline(timelineMap->_size, timelineMap);
				...
				TwoColorTimeline *timeline = creator.createTwoColorTimeline(timelineMap->_size, timelineMap);
	...
	for (boneMap = bones ? bones->_child : 0; boneMap; boneMap = boneMap->_next) {
		...
		for (timelineMap = boneMap->_child; timelineMap; timelineMap = timelineMap->_next) {
			if (strcmp(timelineMap->_name, "rotate") == 0) {
				RotateTimeline *timeline = creator.createRotateTimeline(timelineMap->_size, timelineMap);
				...
				if (isScale || isTranslate || isShear) {
					...
					if (isScale) {
						timeline = creator.createScaleTimeline(timelineMap->_size, timelineMap);
						defaultValue = 1;
					} else if (isTranslate) {
						timeline = creator.createTranslateTimeline(timelineMap->_size, timelineMap);
					} else if (isShear) {
						timeline = creator.createShearTimeline(timelineMap->_size, timelineMap);
					}
					timeline->_boneIndex = boneIndex;
	...
	for (constraintMap = ik ? ik->_child : 0; constraintMap; constraintMap = constraintMap->_next) {
		...
		IkConstraintTimeline *timeline = creator.createIkConstraintTimeline(constraintMap->_size, constraintMap);
	...
	for (constraintMap = transform ? transform->_child : 0; constraintMap; constraintMap = constraintMap->_next) {
		...
		TransformConstraintTimeline *timeline = creator.createTransformConstraintTimeline(constraintMap->_size, constraintMap);
		...
		for (timelineMap = constraintMap->_child; timelineMap; timelineMap = timelineMap->_next) {
			...
			if (strcmp(timelineName, "position") == 0 || strcmp(timelineName, "spacing") == 0) {
				...
				if (strcmp(timelineName, "spacing") == 0) {
					timeline = creator.createPathConstraintSpacingTimeline(timelineMap->_size, timelineMap);

					if (data->_spacingMode == SpacingMode_Length || data->_spacingMode == SpacingMode_Fixed) {
						timelineScale = _scale;
					}
				} else {
					timeline = creator.createPathConstraintPositionTimeline(timelineMap->_size, timelineMap);
				...
				PathConstraintMixTimeline *timeline = creator.createPathConstraintMixTimeline(timelineMap->_size, timelineMap);
				timeline->_pathConstraintIndex = constraintIndex;
				for (valueMap = timelineMap->_child, frameIndex = 0; valueMap; valueMap = valueMap->_next, ++frameIndex) {
					timeline->setFrame(frameIndex, Json::getFloat(valueMap, "time", 0),
		...
		if (skin != NULL)
		{
			for (slotMap = constraintMap->_child; slotMap; slotMap = slotMap->_next) {
				...
				for (timelineMap = slotMap->_child; timelineMap; timelineMap = timelineMap->_next) {
					...
					timeline = creator.createDeformTimeline(timelineMap->_size, timelineMap);
	...
	/** Draw order timeline. */
	if (drawOrder) {
		DrawOrderTimeline *timeline = creator.createDrawOrderTimeline(drawOrder->_size, drawOrder);
	...
	/** Event timeline. */
	if (events) {
		EventTimeline *timeline = creator.createEventTimeline(events->_size, events);
		...
		for (valueMap = events->_child, frameIndex = 0; valueMap; valueMap = valueMap->_next, ++frameIndex) {
			...
			event = creator.createEvent(Json::getFloat(valueMap, "time", 0), *eventData);
	...
	return creator.createAnimation(String(root->_name), timelines, duration);
}
