...
#include "../Skeleton.h"
#include "timelines/RotateTimeline.h"
#include "../data/SkeletonData.h"
#include "../entities/Bone.h"
#include "../data/BoneData.h"
#include "timelines/AttachmentTimeline.h"
#include "timelines/DrawOrderTimeline.h"
#include "timelines/EventTimeline.h"
...
void dummyOnAnimationEventFunc(AnimationState *state, skel::EventType type, TrackEntry *entry, Event *event = NULL) {
	SP_UNUSED(state);
	SP_UNUSED(type);
	SP_UNUSED(entry);
	SP_UNUSED(event);
...
TrackEntry::TrackEntry() : _animation(NULL), _next(NULL), _mixingFrom(NULL), _mixingTo(0), _trackIndex(0), _initialLoopCount(0), _loopCount(0), _holdPrevious(false),
...
std::string TrackEntry::getAnimationName() { return _animation->_name.buffer(); }

bool TrackEntry::getLoop() { return _loopCount != 0; }
...
void TrackEntry::setLoopCount(int count) { _loopCount = count; }
...
float TrackEntry::getAnimationTime() {
	if (_initialLoopCount == 0)
		return _animationStart;
	float duration = _animationEnd - _animationStart;

	float time = MathUtil::fmod(_trackTime, duration);
	if (_loopCount > 0) {
		if (duration == 0 || (_trackLast != -1 && MathUtil::fmod(_trackLast, duration) > time))
			--_loopCount;
	}
...
TrackEntry *AnimationState::setAnimation(size_t trackIndex, const String &animationName, int loopCount) {
	Animation *animation = _data->_skeletonData->findAnimation(animationName);
	assert(animation != NULL);
	return setAnimation(trackIndex, animation, loopCount);
}

TrackEntry *AnimationState::setAnimation(size_t trackIndex, Animation *animation, int loopCount) {
	assert(animation != NULL);
	...
	TrackEntry *entry = newTrackEntry(trackIndex, animation, loopCount, current);
...
TrackEntry *AnimationState::addAnimation(size_t trackIndex, const String &animationName, int loopCount, float delay) {
	Animation *animation = _data->_skeletonData->findAnimation(animationName);
	assert(animation != NULL);
	return addAnimation(trackIndex, animation, loopCount, delay);
}

TrackEntry *AnimationState::addAnimation(size_t trackIndex, Animation *animation, int loopCount, float delay) {
	assert(animation != NULL);
	...
	TrackEntry *entry = newTrackEntry(trackIndex, animation, loopCount, last);
...
void AnimationState::queueEvents(TrackEntry *entry, float animationTime) {
	...
	bool complete = false;
	if (entry->getLoop())
		complete = duration == 0 || (entry->_trackLast != -1 && trackLastWrapped > MathUtil::fmod(entry->_trackTime, duration));
	else
...
TrackEntry *AnimationState::newTrackEntry(size_t trackIndex, Animation *animation, int loopCount, TrackEntry *last) {
	...
	entry._loopCount = loopCount;
	entry._initialLoopCount = loopCount;
