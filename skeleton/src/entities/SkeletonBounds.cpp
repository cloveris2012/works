...
#include "../Skeleton.h"
#include "Bone.h"
#include "attachments/BoundingBoxAttachment.h"
...
void SkeletonBounds::update(Skeleton &skeleton, bool updateAabb) {
	...
	for (size_t i = 0; i < slotCount; i++) {
		...
		skel::Polygon *polygonP = _polygonPool.obtain();
...
bool SkeletonBounds::containsPoint(skel::Polygon *polygon, float x, float y) {
...
bool SkeletonBounds::intersectsSegment(skel::Polygon *polygon, float x1, float y1, float x2, float y2) {
	Vector<float> &vertices = polygon->_vertices;
...
skel::Polygon *SkeletonBounds::getPolygon(BoundingBoxAttachment *attachment) {
...
void SkeletonBounds::aabbCompute() {
	...
	for (size_t i = 0, n = _polygons.size(); i < n; ++i) {
		skel::Polygon *polygon = _polygons[i];
		Vector<float> &vertices = polygon->_vertices;
