...
void RegionAttachment::computeVertices(Bone &bone, float *vertices, size_t offset, size_t stride) {
	for (size_t i = 0; i < _vertexOffset.size(); i += 2) {
		size_t index = (i + BRX) % 8;
		// four vertex coordinates
		vertices[offset] = _vertexOffset[index];
		vertices[offset + 1] = 0;
		vertices[offset + 2] = 0;
		vertices[offset + 3] = 0;

		vertices[offset + 4] = _vertexOffset[index + 1];
		vertices[offset + 5] = 0;
		vertices[offset + 6] = 0;
		vertices[offset + 7] = 0;

		// four index data
		vertices[offset + 8] = bone.getIndex() + 0.1f;
		vertices[offset + 9] = -1;
...
std::string skel::RegionAttachment::getTexName()
{
	return _texName;
}

skel::Color &RegionAttachment::getColor() {
	return _color;
}
